...
public class RollPlugin implements TomPlugin {
  ...
  private final DiceParser diceParser = new DefaultDiceParser();

  private final DiceInterpreter<RollHistory> roller = new DiceRoller();

  @Override
  public Optional<String> resolve(final String expression, final boolean verbose) {
    final RollHistory rolled = diceParser.parse(expression, roller);
